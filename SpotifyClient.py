...
class SpotifyClient:
  ...
  def authorize_app(self, app: App, code, open_browser=False):
    if not os.path.exists('.cache') or open_browser:
      url = f'https://accounts.spotify.com/authorize?client_id={app.client_id}&scope=user-read-playback-state,user-read-currently-playing&response_type=code&redirect_uri=http://127.0.0.1:5000/callback'
